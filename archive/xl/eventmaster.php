...
IF ($LA = mysql_fetch_row($setd)) {
 ...
 IF (!isset($NOHEADS) && ($MINS == 0 && $SECS == 0 && $TENS && $HUNS == 0)) {
   ...
   $RAW = ($T3 * 10) + $HUNS;
     IF ($Division == 'A') {
       $DivName = "Advanced 21'";
       $seta = mysql_query("SELECT * FROM bonusa", $db);
     ...
     IF ($Division == 'B') {
       $DivName = "Advanced 30'";
       $seta = mysql_query("SELECT * FROM bonusb", $db);
       IF ($L = mysql_fetch_row($seta)) {
         do {
           IF ($RAW >= $L[0] && $RAW <= $L[1]) {
             $Bonus = $L[2];
     ...
     IF ($Division == 'C') {
       ...
       $seta = mysql_query("SELECT * FROM bonusc", $db);
       IF ($L = mysql_fetch_row($seta)) {
         do {
           IF ($RAW >= $L[0] && $RAW <= $L[1]) {
             $Bonus = $L[2];
     ...
     IF ($Division == 'D') {
       ...
       $seta = mysql_query("SELECT * FROM bonusd", $db);
       IF ($L = mysql_fetch_row($seta)) {
         do {
           IF ($RAW >= $L[0] && $RAW <= $L[1]) {
             $Bonus = $L[2];
     ...
     IF ($Division == 'E') {
       $DivName = "Beginner 21'";
       $seta = mysql_query("SELECT * FROM bonuse", $db);
       IF ($L = mysql_fetch_row($seta)) {
         do {
           IF ($RAW >= $L[0] && $RAW <= $L[1]) {
             $Bonus = $L[2];
     ...
     IF ($Division == 'F') {
       $DivName = "Beginner 30'";
       $seta = mysql_query("SELECT * FROM bonusf", $db);
       IF ($L = mysql_fetch_row($seta)) {
         do {
           IF ($RAW >= $L[0] && $RAW <= $L[1]) {
             $Bonus = $L[2];
   ...
   $seta = mysql_query("SELECT * FROM timecon", $db); // Figures the acutal score
   //Inserting new code to improve beginners scores
   IF ($Division == 'E' or $Division == 'F') {
     $RAW = $RAW - 3000;//Removes thirty Seconds from Beginner's time before score is figured.
     $BegYes = 1;
     IF ($RAW < 1) {
       print "<H2>Check your input</H2>\n";
       print "<P>You have indicated that a begining rider had a time faster than 30 seconds flat.  This is highly improbable.</P>\n";
     }
   }
   //End Beginner section.
   IF ($L = mysql_fetch_row($seta)) {
     do {
       IF ($RAW >= $L[0] && $RAW <= $L[1]) {
         $Hscore = $Bonus + $L[2] - $Penalty;
       }
     } while ($L = mysql_fetch_row($seta));
   }
 ...
 IF (!isset($NOREEDS)) {
   ...
   print "<B>Having Scored: ".$Hscore." on the Heads Course.</B><BR>\n";
   IF ($BegYes) {
    print "<P>With a 30 second handicap applied</P>\n";
    print "<P>Beginner division scores are granted a handicap when their score is figured.  The script handles this for you.  If your figured score and the scripts do not match up for your beginer scores, see if subtracting 30 seconds fixes the problem before assuming something is amiss.</P>\n";
   }
   IF ($NORINGS) {
    $Rcount = 0;
    $Rscore = 0;
   ...
   print "<B>Having taken ".$Dcount." reeds scording ".$Dscore." for the Reeds Course.</B><BR>\n";
   IF ($NOMOARCH) {
            $Mscore = 0;
   }
   ...
   die;
           ...
           IF ($newdata == "I certify that this score is 100% correct") {
            ...
            $finalscore = $Hscore + $Rscore + $Dscore + $MAscore;
            $champscore = $Rscore + $Dscore;
            $setg = mysql_query("INSERT INTO $pgyear VALUES (NULL, '$Pname', '$Hname', '$EvName', '$SHID', '$SRID', '$SDID', '$SMID', '$champscore', '$finalscore', '$Division', 'N')", $db);
            IF ($setg) {
                echo"done.</P>\n";
            } ELSE {
                echo mysql_error();
...
IF ($Estatus == "O") { //This section only appears if a EMiC can still enter scores.
        ...
        print "</TD></TR>\n";  // END of Horse cell
        print "<TR><TD VALIGN=TOP ALIGN=CENTER COLSPAN=4>\n"; // Division Cell
        print "<TABLE BORDER=0><TR><TD>\n";
        ...
        print "</TD></TR></TABLE>\n";
        print "</TD></TR>\n"; // End of Divison Cell
        print "<TR><TD WIDTH=30% VALIGN=TOP ALIGN=CENTER>\n"; // Heads Cell
    ...
    print "<input name=\"Penalty\" type=\"text\" size=\"2\" value=\"00\">\n";
        print "</TD></TR></TABLE>\n";
        print "</TD>\n"; // End of Heads Cell
        print "<TD WIDTH=30% VALIGN=TOP ALIGN=CENTER>\n"; // Rings Cell
    ...
    print "</TD></TR>\n";
    print "</TABLE>\n"; // END of MoARCH Cell
    print "<TR><TD COLSPAN=4 ALIGN=CENTER><input type=\"submit\" name=\"newdata\" value=\"Click here to add this score\">\n";
...
$setb = @mysql_query("SELECT PName,HName,Ename,SHscore,SRcount,SRscore,SDcount,SDscore,score,$pgyear.DIV,YID,seen,$pgyear.PID,$pgyear.HID,SMscore,cscore FROM $pgyear LEFT JOIN riders ON $pgyear.PID=riders.PID LEFT JOIN events ON $pgyear.EID=events.EID LEFT JOIN horses ON $pgyear.HID=horses.HID LEFT JOIN heads ON $pgyear.SHID=heads.SHID LEFT JOIN rings ON $pgyear.SRID=rings.SRID LEFT JOIN reeds ON $pgyear.SDID=reeds.SDID LEFT JOIN moarch ON $pgyear.SMID=moarch.SMID WHERE $filterby", $db);
IF ($L = mysql_fetch_row($setb)) {
        do {
        ...
        switch ($temp) {
            ...
            printf("<TD>%s</TD><TD>%s</TD><TD>%s</TD><TD>%s</TD><TD>%s / %s</TD><TD>%s  / %s</TD><TD>%s</TD><TD>%s</TD><TD>%s</TD><TD>%s</TD>", stripslashes($L[0]), stripslashes($L[1]), stripslashes($L[2]), $Lheads, $L[4], $Lrings, $L[6], $Lreeds, $Lmoarch, $L[15], $L[8], $SDiv);
        $Apcount = 0;
        $temp2 = $L[11];
                switch ($temp2) {
                        case "D":
...
print "<P>This page may look a little different this year, with the changes to the games.  The championship rests on the Reeds and Rings games, and the table above referances that score as I: and AG: The values in these blanks are for the IKEqC: and All Games: blanks just like on your scoresheets.</P><P>Key for abbreviations:</P>\n";
print "<P><B>H:</B> is for Heads.<BR><B>R:</B> is for Rings.<BR><B>D:</B> is for Reeds.<BR><B>M:</B> is for Mounted Archery.<BR><B>I:</B> is for IKEqC score total.<BR><B>A:</B> is for the total for All Games offered at this event.</P>\n";
print "<P>You can contact Sandor about technical problems or matters at <A HREF=\"mailto:sandor_dosa@yahoo.com\">sandor_dosa</a><A HREF=\"mailto:sandor_dosa@yahoo.com\">@</A><A HREF=\"mailto:sandor_dosa@yahoo.com\">yahoo.com</A>.\n";
print "<P>You can contact THL Donweena Beasthealer at <A HREF=\"mailto:drdonna@pioneernet.net\">drdonna</A><A HREF=\"mailto:drdonna@pioneernet.net\">@</A><A HREF=\"mailto:drdonna@pioneernet.net\">pioneernet.net</A>.\n";
print "</TD></TR>\n";
?>
</TABLE>
</BLOCKQUOTE>
